...
export function buildMasterTools(orchestrator: Orchestrator): PimClawTool[] {
  return [
    {
    ...
    },
    {
      ...
      execute: async (args) => {
        const entry = await orchestrator.createAgent(
          args.role as string,
          args.name as string,
          (args.mcpServices as Record<string, { command: string; args: string[]; env?: Record<string, string> }>) ?? {},
        );
        return {
    ...
    },
    {
      ...
      execute: async (args) => {
        const ok = await orchestrator.terminateAgent(args.agentId as string);
        return {
    ...
    },
    {
      ...
      execute: async (args) => {
        const entry = orchestrator.getAgent(args.agentId as string);
        if (!entry) {
    ...
    },
    {
      ...
      execute: async (args) => {
        const result = await orchestrator.routeTask(
          args.task as string,
          args.targetAgentId as string | undefined,
        );
        return {
    ...
    },
    {
      ...
      execute: async (args) => {
        const result = await orchestrator.callAgentMcpTool(
    ...
    },
    {
      ...
      execute: async (args) => {
        const tools = await orchestrator.listAgentTools(args.agentId as string);
        return {
...
export function createPimClawMcpServer(orchestrator: Orchestrator): Server {
  const tools = buildMasterTools(orchestrator);
  const toolMap = new Map<string, PimClawTool>();
  for (const tool of tools) {
    toolMap.set(tool.name, tool);
  }

  const server = new Server(
    { name: "pimclaw", version: "0.1.0" },
    { capabilities: { tools: {} } },
  );
 ...
 */
export async function servePimClawMcp(): Promise<void> {
